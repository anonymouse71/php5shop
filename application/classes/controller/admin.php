...
class Controller_Admin extends Controller_Template {
             ...
             {                 
                 ...
                 foreach($orders as $k => $item)
                 {
                     ...
                     if($item->user)
                     {
                         ...
                         $this->template->body->array[$k]['user_name'] = ORM::factory('user',$item->user)->__get('username');
                     }
                     $this->template->body->array[$k]['phone'] = $item->phone;
                     $this->template->body->array[$k]['status'] = isset($state[$item->status])? $state[$item->status] : 'название этого статуса удалено';
    ...
    public function action_config()
    {
        ...
        $this->template->body->answers = ORM::factory('poll_answer')->find_all();
                                                                                //обработка журнала логов
//        $basePathLogs = $_SERVER['DOCUMENT_ROOT'] . url::base() . 'application/logs/' . date("Y/m/");
//        $errorLogs = '';
//        for($i=32;$i>0;$i--)
//        {
//            $day = ($i < 10)? "0$i" : $i;
//            if(is_file($basePathLogs . $day . '.php'))
//            {
//                $errorLogs .= '<br><br>' . $day . strip_tags(file_get_contents($basePathLogs . $day . '.php'));
//            }
//        }
//        $errorLogs = nl2br($errorLogs);
//        if(mb_strlen($errorLogs,'UTF-8') > 0)
//        {
//            $this->template->body = View::factory('admin/info')->set('info',
//                    '<a href="javascript:void(0);" id="readerrors">Читать журнал ошибок за этот месяц</a>' .
//                    ' <br>(полный журнал в директории application/logs/ )' .
//                    '<div id="datalog">' . mb_substr($errorLogs,0,1000,'UTF-8') . '</div>' .              //скрытый div с содержанием всего журнала логов
//                    '<br>Журнал хранит информацию об ошибках, которые могли быть вызваны ' .
//                    'неполадками на сервере или ошибками в движке магазина. ' .
//                    'Если Вы не знаете значение записанной ошибки, обратитесь к разработчику.')
//                    ->render() . $this->template->body;
//        }
