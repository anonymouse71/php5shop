...
class Controller_Shop extends Controller_Template {
    ...
    public function before()
    {
        ...
        if(strlen(Session::instance()->get('currency')) === 3)                  //если в сессии есть 3-буквенный банковский код, устанавливаем его как валюту
        ...
        if($this->boolConfigs['currency'])                                      //согласно настройкам
        {
            ...
            $tpl->topBlock1->array = array_keys($currency);                     //подстановка в шаблон переменной с массивом названий (банковских кодов) валют
        }

        $this->auth = Auth::instance();                                         //инициализация механизма авторизации
        
        if($this->boolConfigs['LoginOn'])                                       //если функция авторизации включена
        {
            $tpl->loginForm = new View(TPL . 'loginForm');                      //подключаем шаблон формы авторизации.
            if(Session::instance()->get('login_error') == 1)                    //если пользователь уже пробовал авторизоваться и допустил ошибку, о которой свидетельствует COOKIES
            {
                $tpl->loginForm .= new View('modalLoginError');                 //добавляем всплывающее окно
                Session::instance()->delete('login_error');                     //удаляем переменную из COOKIES чтобы сообщеие больше не повторялось (до следующей ошибки авторизации)
            }
            ...
            if(Session::instance()->get('password_changed') == 1)               //если пользователь использовал систему восстановления пароля
            {
                $tpl->loginForm .= new View('passwordsend');                    //добавляем всплывающее окно
                Session::instance()->delete('password_changed');                //удаляем переменную из COOKIES чтобы сообщеие больше не повторялось
            }

            if(!$this->auth->logged_in())                                       //если пользователь не авторизован
                $this->auth->auto_login();                                      //инициализации автовхода по COOKIES.
        ...
        else                                                                    //если функция авторизации отключена
        {
            $tpl->menu[6] = FALSE;                                              //прячем кнопку "Аккаунт" из меню
            $tpl->loginForm = '';                                               //переменную для формы авторизации инициализируем значением ""
        }
    ...
    public function action_index()                                              //главная страница
    {
            ...
            {                                                                   //учитываем скидку, курс валют и добавляем к цене банковский код валюты:
                ...
                if(is_array($sessionCart))                                      //но если корзина записана как массив
                    if(in_array($product['id'], $sessionCart))                  //и продукт в ней записан
                        $product['cart'] = TRUE;                                //передадим эту информацию в шаблон
        ...
        elseif($cat)                                                            //указана категория
        {
            ...
            if(!$productsCount)                                                 //нет товаров в категории
                $this->template->stuff = 'В этой категории сейчас нет товаров.';//вставляем сообщение
            else
            {
                ...
                {                                                               //учитываем скидку, курс валют и добавляем к цене банковский код валюты:
                    ...
                    if($this->boolConfigs['bigCart'])                           //если в настройках пользователю разрешено выбирать количество товаров
                    {
                        if(isset($bcart[$p['id']]))                             //если товара выбрано больше 1 ед.
                            $products[$k]['bigcart'] = $bcart[$p['id']];        //вставляем колич. ед. в массив
                        else                                                    //иначе если
                            if(in_array($p['id'],$sessionCart))                 //товар выбран в количестве 1 ед.
                                $products[$k]['bigcart'] = 1;                   //записываем колич. 1
                    }
    ...
    public function action_register()
    {
        ...
        $this->template->about->val = Session::instance()->get('register_post');//подключение в шаблон данных, которые были введены в прошлый раз (если тогда были ошибки)
    ...
    public function action_user()
    {
        ...
        if($pct != 1)                                                           //если он не равен 1
            $this->template->about->pct = (100 * (1 - $pct)) .  '%';            //подставляем в представление
        ...
        if($this->boolConfigs['refpp'])
        {
            ...
            $this->template->about->info = str_replace('{{link}}', (isset($_SERVER['HTTPS'])? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . url::base() . $this->user->id, Model::factory('affiliate')->about());
        }
    ...
    public function action_blog($id=0)
    {
        ...
        if($id)                                                                 //если он установлен
        {
            ...
            if(!isset($view->post->id))                                         //нет записи с таким id?
                exit($this->request->redirect(url::base().'error/404'));        //перенаправление на страницу 404
        ...
        {
            $page = isset($_GET['page'])? abs((int) $_GET['page']) : 0;         //получение GET параметра page с установкой его >= 0
            ...
            {                                                                   //и добавляем в $this->template->about
                ...
                if($view->post->html2)//строка на случай если ф-я комментирования добавлена в версию движка "на гарячую"
                    $view->post->html = $view->post->html2;                     //показываем сокращенныую новость вместо полной
            ...
            $this->template->about .= new Pagination(array(
                'uri_segment'    => 'page', 
                'total_items'    => Model::factory('BlogPost')->find_all()->count(),
    ...
    {                                                                           //в настройках машрутизации регулярное выражение [A-Z]{3}
        ...
        if(FALSE === strpos(Request::$referrer,'://' . $_SERVER['HTTP_HOST']))   //если HTTP_REFERER содержит домен сайта
    ...
    public function action_sortset($code=0)
    {
        ...
        $this->request->redirect(url::base() . 'shop/category'
    ...
    public function action_order($phone=null)
    {
        ...
        $to = array();
        if($this->boolConfigs['ordMail'] && isset($emails[1]))                  //найден email и в настройках установлено отправлять на него
            $to['email'] = $emails[1];
        if($this->boolConfigs['ordJabb'] && isset($emails[2]))                  //найден jabber и в настройках установлено отправлять на него
            $to['jabber'] = $emails[2];
        ...
        if(!is_array($products) || !count($products))                           //если продуктов там нет
            $this->request->redirect(url::base());                              //перенаправление
        ...
        if(Auth::instance()->logged_in())                                       //пользователь авторизован?
        {
            ...
            if(isset($user['phone']) && $way && isset($_POST['confirm']))       //есть все необходимое для заказа
                $message = Model_Order::create($products, $user, $to, $way, $counts);//заказ сохранен
        ...
        elseif($phone && strlen($phone)>=6 && strlen($phone)<=12 && !$stop && $way && isset($_POST['confirm']))             
            $message = Model_Order::create(
                    $products, array('phone'=>$phone), $to, $way, $counts);     //заказ сохранен
        elseif(!$phone)
            ...
            {                                                                   //учитываем скидку, курс валют и добавляем к цене банковский код валюты:
                ...
                if(is_array($sessionCart))                                      //но если корзина записана как массив
                    if(in_array($product1['id'], $sessionCart))                 //и продукт в ней записан
                        $product1['cart'] = TRUE;                               //передадим эту информацию в шаблон
                ...
                if(is_array($sessionCart))                                      //но если корзина записана как массив
                    if(in_array($product2['id'], $sessionCart))                 //и продукт в ней записан
                        $product2['cart'] = TRUE;                               //передадим эту информацию в шаблон
    ...
    {
        parent::after();
        if($this->boolConfigs['timeFooter'] && isset($this->template->banner4)) //если в настройках это включено, в footer добавляется Benchmark
