...
class Controller_Login extends Controller {
    ...
    {
        $this->checkLogin();                                                    //checkLogin проводит проверки на то, включена ли ф-я авторизации и не авторизован ли уже пользователь, есть ли POST массив
        ...
        if(isset($_POST['login']) && isset($_POST['pass']))                     //если логин и пароль переданы
            ...
            {
                $antibrut->bad();
                Session::instance()->set('login_error', '1');                   //В случае ошибки, информация о ней записывается в COOKIES
            }

        $this->request->redirect(                                               //Перенаправление на страницу с которой пользователь пришел (если она на этом домене)
                        (
    ...
    public function action_register()
    {
        ...
        $group = isset($_POST['finvite']) && strlen($_POST['finvite']) ? Model_Invite::checkInvite($_POST['finvite']) : 0;
        
        if(isset($_POST['captcha']) && Captcha::valid($_POST['captcha']))       //если проверочное изображение введено верно,
        {
            $register = Model::factory('user')->register($_POST,TRUE,$group);   //пользователь добавляется, а массив ошибок или TRUE записывается в переменную $register
        ...
        {                                                                       //происходит проверка остальных полей
            $register = Model::factory('user')->register($_POST,FALSE,$group);  //без занесения инфрмации в БД, даже если все остальные поля введены верно
        ...
        if($register === TRUE)                                                  //если пользователь успешно зарегистрирован
        {
            ...
            if(EMAIL_VERIFY)
            {
                ...
                $this->auth->login($_POST['username'],$_POST['password'],TRUE); //авторизуем
    ...
    {
        if( isset($_POST['exit']) && Auth::instance()->logged_in() )            //если пользователь вошел и попал на страницу нажатием на кнопку "Выход"
            Auth::instance()->logout(TRUE);                                     //сессия уничтожается
        $this->request->redirect(url::base());                                  //в любом случае на этой странице делать больше нечего - перенаправим на главную
    }
    ...
    public function action_emailpass()
    {
        ...
        {                                                                     //где num > 0
            ...
            {                                                                   //в сессию записывается информация об ошибке и происходит перенаправление
                Session::instance()->set('emailpass_errors', 'Вы зашли с другого браузера или сессия закончилась. Попробуйте отправить новый запрос на смену пароля.');
            ...
            Session::instance()->set('password_changed',1);                     //установка в сессию переменной, индикатора завершения смены пароля
        ...
        {
            Session::instance()->set('emailpass_errors', 'Email не настоящий'); //в сессию записывается информация об ошибке и
            exit($this->request->redirect(url::base().'shop/forgotpassword'));  //происходит перенаправление
        }
        else if(!$chkEmail)                                                     //нет пользователя с таким email
            {
        ...
        Session::instance()->regenerate();                                      //пересоздание сессии чтобы у пользователя на активацию было ровно столько времени, сколько живет сессия
    ...
    private function checkLogin()                                               
    {
        ...
        if(!$boolConfigs['LoginOn'] /*|| !count($_POST)*/)
            exit( Request::factory('error/404')->execute() );                   //завершаем выполнение ошибкой 404 (Not found).
    ...
    /**
     * Альтернативная авторизация для администратора
     *  на случай выключения опции "авторизация для пользователей".
     * Адрес страницы /login/admin/{имя_администратора}
     * @param string $username
     */
    public function action_admin($username=null)
    {
        ...
        $id = $model->find_id($username);
        if($model->is_admin($id))
        {
            if(isset($_POST['pass']))
            {
                $antibrut = Model::factory('Antibrut');                         //механизм защиты от перебора паролей
                if(!$antibrut->chk())
                    exit( Request::factory('error/loginlimit')->execute() );    //перенаправить пользователя на страницу ошибки.
                if($this->auth->login($username, $_POST['pass'], TRUE))
                    exit($this->request->redirect(url::base()));
                else
                    $antibrut->bad();
            }
            $this->request->response = new View('admin/altLogin');
        }
        else
