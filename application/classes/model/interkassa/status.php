...
class Model_Interkassa_Status
{
     ...
     * @param array $source
     *
     * @see Model_Interkassa_Status::__constructor()
     *
     * @return Model_Interkassa_Status
     */
    public static function factory(Model_Interkassa_Shop $shop, array $source)
    {
        return new Model_Interkassa_Status($shop, $source);
    }
     ...
     *
     * @throws Model_Interkassa_Exception if some data not received, received shop id
     *                              does not match current shop id or received
     *                              signature is invalid
     */
    public function __construct(Model_Interkassa_Shop $shop, array $source)
    {
        $this->_shop = $shop;
        ...
        foreach (array(
            ...
            if (!isset($source[$field]))
                throw new Model_Interkassa_Exception($title . ' not received');

        $received_id = strtoupper($source['ik_co_id']);
        $shop_id = strtoupper($shop->getId());

        if ($received_id !== $shop_id)
            throw new Model_Interkassa_Exception('Received shop id does not match current shop id');

        if ($this->_checkSignature($source))
            $this->_verified = true;
        else
            throw new Model_Interkassa_Exception('Signature does not match the data');
     ...
     *
     * Returns {@link Model_Interkassa::STATE_SUCCESS} or {@link Model_Interkassa::STATE_FAIL}
     *
     * @return string
     */
